...
class MailboxesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create mailbox" do
    assert_difference('Mailbox.count') do
      post mailboxes_url, params: { mailbox: { name: @mailbox.name, note: @mailbox.note, token: @mailbox.token } }
    end
  ...
  test "should update mailbox" do
    patch mailbox_url(@mailbox), params: { mailbox: { name: @mailbox.name, note: @mailbox.note, token: @mailbox.token } }
